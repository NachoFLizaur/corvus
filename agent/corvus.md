...
When `tests_enabled: true, tests_deferred: false`: code-quality reports test results, acceptance verification, task attribution for failures (at every phase gate).
When `tests_enabled: true, tests_deferred: true`: code-quality reports acceptance verification only during Phase 4 (acceptance-only mode). Phase 5a runs the full test suite.
When `tests_enabled: false`: code-quality reports acceptance verification and task attribution only (acceptance-only mode). No tests at any phase.
...
[Plan-Type Selection] ◄── NEW STEP
    ...
    │
    ▼
Corvus asks: "Generate tests?" (via question() tool — 3 options)
    │
    ▼
...
**Goal**: Ask user whether and when tests should be generated and run.
...
- options:
  1. label: "Yes (recommended)", description: "Generate test tasks and run tests at every quality gate (default)"
  2. label: "Yes — at end only", description: "Generate test tasks but defer testing to Phase 5 final validation. Quality gates use acceptance-only mode during Phase 4"
  3. label: "No — skip tests", description: "Skip test generation entirely, quality gates use acceptance-only mode"
...
Pass both flags to Phase 2 (task-planner) via the delegation template's `**TEST PREFERENCE**` field.

> **Note**: This is a lightweight step — just a question tool call, no subagent delegation needed.
> **Note**: The "Yes — at end only" option matches the behavior hardcoded in corvus-auto. Tests are generated by task-planner but Phase 4 quality gates run in acceptance-only mode. Phase 5 runs the full test suite.
...
- **5a**: code-quality (ALWAYS — tests + acceptance when `tests_enabled: true` (including deferred mode, which runs full tests here for the first time), acceptance-only when `tests_enabled: false`)
...
18. `tests_enabled` and `tests_deferred` flags jointly control test generation and execution mode
...
24. When `tests_deferred: true`, Phase 5 is mandatory even for Lightweight plans (deferred tests must run somewhere)
