...
<!-- Dispatch invariant: live pr.metadata for the posting-authorization comparison below, trusted identity/config controls, final approval/mode, complete axis/projection evidence, rendered document, and inline count are read before writer dispatch. Any control mismatch fails local-only; previous local-only is terminal. No override, edited finding, or empty result disables the checks. -->
