...
1. Require post in interactive mode with explicit authorization after the final frozen preview, or auto_post in autonomous mode; decision_reason and rails_applied are present, with no pending edit/rerun. Done when authorization refers to this exact artifact digest and event, not just the full local document.
...
After revalidation, require only the current frozen [POST_ARTIFACT](../corvus-review-extras/schemas.md#post_request-and-post_result--r5writer) from R4 or Anchor Relocation below to match current identity/head and the approved event, with authorization bound to its previewed digest. Keep that descriptor's expected_sha256 unchanged for dispatch, including each permitted re-dispatch. Repair missing/invalid descriptors through [R4 Artifact Repair](../corvus-review-r4/SKILL.md#artifact-repair); incomplete analysis evidence stays in Review limits. If the writer is absent from host inventory, record `writer_capability_not_exposed`. Done when authorization binds the frozen bytes or the observed host exception applies.

Call `corvus_review_pr` op `reviews` before dispatch to retain available baseline review IDs; missing history is disclosed, not a delivery gate. Retain artifact bytes and descriptor unchanged during transport recovery; Anchor Relocation or R4 Artifact Repair owns replacement.
...
```
Include no review body text, comments, findings, TASK block, or extra controls in the dispatch. The writer validates the descriptor, reads artifact controls and current head/anchors, performs marker lookup before `corvus_review_post`, then maps TransportResult into POST_RESULT without retyping content. R3/R4 own payload measure/freeze/preview; the writer's `corvus_review_post` owns posting integrity. Done when one authorized artifact descriptor has been dispatched.
...
| Valid POST_RESULT not_posted, reason verify: <diagnostic> (including artifact-verify-failed:<detail>) | Return to [R4 Artifact Repair](../corvus-review-r4/SKILL.md#artifact-repair), not transport recovery; retain the tool diagnostic and remote_state not_posted |
...
- Complete evidence proving no matching review after unknown/malformed transport: make ONE more writer dispatch with the identical descriptor. Track `transport_repost_attempted` for the run and preserve it across re-dispatches; the writer checks pr.reviews for the marker before POST to avoid repeating visible delivery. Reconcile again after that dispatch, without another ambiguous-transport POST. R5 owns any additional dispatch after ambiguous transport.
...
On `anchors-unverifiable`, match each unique `{path,line_start,line_end}` in `unverifiable_anchors` to the frozen artifact's previewed comments and REVIEW_DOCUMENT (start_line or line through line); reject unknown positions. Relocate ONLY those matching inline comments into their own axis's review body using the existing [identity-preserving Conventional Comments format](../corvus-review-extras/SKILL.md#conventional-comments), adding the original path/range while preserving each entire comment body, identity and suggestion unchanged. Keep all other inline comments, their order, findings, action, marker, notices and counts unchanged before freeze fitting; increment R5's cumulative `comments_moved_to_body` by the number moved, not the number of unique anchors, and retain it independently of the writer's zero count and freeze's `omitted` counts bound to the current artifact digest.
For this new presentation revision, invalidate the old descriptor and run stage → measure → freeze → `preview` → renew authorization (interactive: question on the previewed replacement; autonomous: re-emit `auto_post` bound to the new digest) → revalidate → dispatch.
Use R3 for the best-effort revised complete checkpoint and candidate staging/measurement, then [R4 Preflight](../corvus-review-r4/SKILL.md#preflight) for freeze/preview and the invoking mode's renewed authorization; a missing question follows the state procedure. This is not a transport retry. Retry while progress is made; each matched relocation removes those inline anchors while preserving their full evidence locally. The writer never edits artifacts. Artifact failures use R4 Artifact Repair; unknown transport uses reconciliation, preserving `transport_repost_attempted` and the separate relocation/omission accounting.
...
- Posted: review URL, constrained action, coverage/notices, separate axis totals/key concerns, actual inline/moved counts, `size_fit` when applied and separate `omitted.comments`/`omitted.findings` from the delivered artifact, filters, and series evidence; distinguish full local counts from posted content.
