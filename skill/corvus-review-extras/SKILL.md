...
## Operating Rules
### Delivery Principle
<!-- Delivery invariant: trusted invocation, available synthesis, live PR state and observed writer outcome are read before delivery decisions. Evidence gaps fail toward disclosed COMMENT, not silence; only the five exceptions below disable delivery. Tool validation still rejects invalid bytes; repair and re-freeze rather than bypassing validation. -->
Delivery is the default. Once R3 synthesis exists, the review IS posted in autonomous mode (COMMENT event when identity/config cap applies). The ONLY reasons not to post: LOCAL mode (no PR); PR state CLOSED/MERGED at R5 revalidation; the writer agent or its required tools are not exposed by the host; GitHub rejected the POST after the writer's attempt; frozen artifact fails verification after re-freeze. Everything else — coverage gaps, missing/incomplete evidence, child errors or malformed reports, unresolved questions, cap overflows, checkpoint persistence failure, sync failure, verdict failure, prior-review history gaps — is DISCLOSED in the review's `Review limits` section and never blocks posting. Retries have no fixed cap: retry while each attempt makes progress (different error or more evidence); stop retrying only when two consecutive attempts yield the identical result, then proceed with what exists.
Interactive posting still requires the user's authorization; failures never switch invocation mode. The explicit verification/re-freeze and ambiguous-POST recovery checkpoints in R4/R5 are exceptions to the general retry policy, not limits on information gathering.
...
<!-- Action invariant: identity/config provenance, available axis/projection evidence and inline count are read before R4 and R5. Unknown authority for an opinion caps its event to COMMENT; gaps never grant APPROVE. Verified skips disable only review work; no override removes notices or the Delivery Principle. -->
...
3. **Coverage caps**: failed or skipped requires informational `COMMENT_ONLY`; partial permits `REQUEST_CHANGES` only with a retained unsuppressed blocker/critical and otherwise requires `COMMENT_ONLY`. Partial never approves. Complete has no coverage cap.
...
R0 owns trust inputs; R1 owns context recovery; R2 owns contribution statuses/retries; R3 owns synthesis; R4 owns authorization and re-freeze; R5 owns writer transport recovery and cleanup. Retry while progress is made, continue with independently valid evidence, and carry gaps into Review limits. Done when the result accounts for evidence gaps and remote-state uncertainty under the Delivery Principle.
