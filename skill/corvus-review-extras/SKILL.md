...
Missing/duplicate dimensions, unknown statuses, absent reasons, or inconsistent axis/projection data are invalid. Preserve contribution summaries and no-spec/reduced-coverage limitations even when the projection completes. Coverage notices are derived controls: edits change findings, not coverage evidence.
...
1. **Trust and no-post rails**: invalid/missing/contradictory controls, failed trust, previous local-only state, or `inline_comments.length > safety_rail_threshold` force `local_only`. Autonomous mode also forces local-only for any projected `error`; successful sibling findings remain visible locally.
...
|----------|-------|---------|
| 5 | blocker | Release-stopping defect |
| 4 | critical | Significant high-impact defect |
| 3 | major | Actionable defect |
| 2 | minor | Small actionable improvement |
...
R0 owns trust failures; R1 owns context recovery; R2 owns contribution statuses/retries; R3 owns synthesis failure; R4 owns authorization; R5 owns writer transport recovery and terminal cleanup. Use the owning procedure instead of inventing recovery in a caller. Done when the terminal result accounts for evidence gaps and any remote-state uncertainty.
