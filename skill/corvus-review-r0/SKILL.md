...
For a bare number or `#number` without a trusted repository, resolve owner/name via `corvus_review_pr` op `repo`; retry a failed read once. On exhaustion, retain the diagnostic and try `local` for current-worktree review with validated workspace identity, not a guessed PR target. `no-repository` means the PR locator needs owner/repo, not that local review is unavailable.
...
Pass branch names as tool data, never shell substitutions. A successful `find` returns either `{found: true, number, url, state}` or `{found: false, candidates}`. Validate a discovered locator as above before metadata. With multiple candidates, interactive mode may ask once; absent a selection, both modes choose OPEN first, then greatest PR number as the recency proxy/tie-break, recording the assumption. Empty candidates select LOCAL through `{op: "local"}`. Retry a discovery error once, then try LOCAL with a PR-discovery-unavailable note; an error is not confirmed PR absence and cannot use the empty-result message.
...
## LOCAL Intake
<!-- LOCAL invariant: successful discovery/local results and validated workspace identity are read before namespace writes or dispatch. Invalid identity/OIDs or unavailable diff evidence fail local-only; missing input disables no checks. Only a validated PR locator selects PR semantics; LOCAL never enables posting. -->
...
Require the returned number and canonical URL's owner/repo to match the candidate, and baseRefOid/headRefOid/code_head to be full 40-hex SHAs; normalize to lowercase base_sha/head_sha/code_head. Use code_head as review identity; head_sha is the observed raw tip. Failure terminates `failed`/`local_only` before R1, releasing only an owned lock if acquired. Done when PR_CONTEXT has verified identity and all OIDs.
...
<!-- Gatherer boundary: read completeness and provenance before recovery or R2 dispatch. Missing evidence stays a gap; after one re-dispatch in either mode, direct read-only gathering may supplement available evidence. Unverified head/cleanliness disables pointers, not review; no fallback disables provenance or retry limits. -->
Prefer `pr-context-gatherer` for worktree cleanliness and changed content. A failed/truncated report gets one re-dispatch for the missing remainder, preserving completed evidence; this supersedes R1's retry counts. Then supplement through granted read-only tools/bash, attributing sources, or continue with explicit gaps under R1's evidence gate. Done when provenance and recovery are accounted for.
...
Then call `corvus_review_verdict` with `{op: "compute", reviewRoot: review_root, ...<state verdict identity>, priorReviews, config: {}, forceDelta: <trusted invocation force_delta or false>}` without headSha or code_head (history-only). Use its round, never a claimed round. Before R1/R2, record `refuse_delta`/refuse_reason unchanged and follow shared Convergence and Continuation: review once with a note, using fresh full-review scope for missing history. Retry tool failure once, then continue available analysis with round unknown and the diagnostic; never invent force_delta or convergence. Exact-head post/resume compares code_head under state validation without admitting a new delta.
...
| Verified empty PR diff | Review skipped locally; LOCAL uses its intake empty-result condition |

For rate-limited reads, retry once; exhausted reads continue with available evidence and retrieval notes, using config defaults where applicable. Identity/OID validation still gates PR dispatch; identity/prior-review/CI gaps retain their caps and limitations. Every terminal branch calls `corvus_review_lock` op `release` for this run under the state contract and discloses cleanup failures.
