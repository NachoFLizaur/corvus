...
At R2 entry, serialize assembled REVIEW_CONTEXT plus R0 evidence into the closed [REVIEW_INPUT-file schema](../corvus-review-extras/schemas.md#review_input--r2-children), pretty-printed with every long value chunked per that schema. Under the owned lock, assemble `<review_root>/review-input.json` with the same ≤20,000-character bounded sequential patches as [Persist at R3 step 2](../corvus-review-extras/state.md#persist-at-r3), then JSON-validate the complete read-back before either child launches. Validate identity/head and schema with the read tool, paging through the entire file, checking the first/last lines and required keys, and confirm no line is longer than 1,900 characters — the read tool truncates longer lines, so if any exists, rewrite the file chunked — never an interpreter or shell (`bun -e`, `node -e`, `python`, `jq`), which the host denies and which adds no evidence the read tool lacks. Reuse unchanged for both children and retries/reruns of that R1 assembly. A write that succeeds and reads back as complete valid JSON is accepted whatever its size — an oversized successful write is logged, not failed, per the shared [Operating Rules](../corvus-review-extras/SKILL.md#operating-rules). A write error, read-back truncation, parse failure or missing required write argument gets one smaller assembly retry via the compaction ladder and the same bounded procedure; partial JSON is never consumable by children. A second genuine failure or a denial terminates local-only with path, last payload size and tool diagnostic; never fall back to inline dispatch.
