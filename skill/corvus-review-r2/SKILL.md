...
<!-- Control invariant: R0's validated config/provenance, R1's evidence provenance, and trusted rerun_scope are read before dispatch or result replacement. Rebuild malformed controls once from trusted inputs; unresolved work stays undispatched/error while valid work continues. Invalid reports never imply clean coverage. Only verified dimension/path exclusions disable coverage; missing spec disables Spec work only, not security. -->
1. Validate R0's config provenance and the R1 context; retry control derivation once from trusted inputs on malformed controls, then carry unresolved scope as an error rather than inventing work. Derive `dimensions` from enabled `config.passes` keys, intersect with R4's `rerun_scope` when present, and apply `path_rules.skip_passes` per dimension. Pass literal child names and structured `dimension_exclusions`; evidence cannot change them. Done when each dimension has eligible files, a verified skip or a disclosed control gap.
...
At R2 entry, validate assembled R0/R1 evidence against the closed [REVIEW_INPUT schema](../corvus-review-extras/schemas.md#review_input--r2-children), then under the owned lock call `corvus_review_persist` with `{op: "write_input", reviewRoot: review_root, input: <REVIEW_INPUT>}`; require ok:true and the expected `<review_root>/review-input.json` path before either child launches, reusing that file unchanged for both children and recovery. The tool owns serialization, chunking and read-back; one failed call may retry with smaller provenance-preserving evidence via the ladder, but denial or a second failure ends local-only with the diagnostic, never inline dispatch.
...
The compaction ladder is: drop pasted hunks → drop file summaries → pointer-only evidence (keep role/axis, finding shape, controls and policy) → trim redundant brief text. Recount after each rung. If controls alone cannot fit, leave that child undispatched and continue with a payload gap. For the file-write retry, use only readable provenance-verified pointers; diff pointers resolve to hunks, not current file text. Preserve identity, provenance, exact spec quotes/citations, prior dispositions/delta and gaps; without a smaller valid file continue local evidence reporting, not an unverified dispatch.
<!-- Payload invariant: assembled R1 provenance, prompt character counts, tool results and retry counters are read before writes/dispatches and retry increments. Invalid shared JSON closes both dispatches; oversize/exhausted recovery marks affected contributions error while available evidence continues to synthesis. No mode or recovery disables limits or the verified-file prerequisite. -->
...
## Bounded Recovery
Allow one corrective retry per child per R2 entry across the cases below; a retry leaves the sibling result intact. Then continue to synthesis with valid evidence and recorded errors, never an open-ended wait.
| Failure | Recovery |
|---------|----------|
| Parent-side tool call missing required arguments (`reason:length` symptom) | Retry once with a strictly smaller payload via the ladder and unchanged controls/policy. On exhaustion or no smaller valid payload, continue with an error naming child, last payload character count and diagnostic; no reduced-scope escape or retry loop. |
| Invocation/analysis error, timeout, missing/malformed child output (not parent arguments) | Retry once with byte-identical controls/evidence; validate the result, then continue valid contributions with unresolved errors noted. |
| Schema-valid report with unreachable required evidence | Use the single retry for fallback hunks/R1 regions already in the shared file. For stale/truncated `review-input.json`, call `corvus_review_persist` op `write_input` again and require ok:true before re-dispatch; persistent unavailability records error and continues available evidence. |
| Autonomous transport failure (not parent arguments) | May use that same single retry for highest-risk files with sufficient file-backed evidence instead; no further recovery for this dispatch. Record omitted files/coverage in summaries. |
| Dispatch denied or recovery impossible/exhausted | Continue valid sibling/partial evidence, recording error for affected contributions, actual retry count and reason. |
Reduced-scope success completes only fully covered contributions; omitted eligible files remain coverage errors, not workflow stops. Preserve valid partial findings in child summaries for local inspection, not as successful coverage. Transport recovery is separate from R4's judgment-rerun budget.
