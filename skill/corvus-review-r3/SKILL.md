...
Persist the full REVIEW_DOCUMENT checkpoint via [Persist at R3](../corvus-review-extras/state.md#persist-at-r3) BEFORE measurement, including both axis maps/projection, final tags, edit history, and exact strings. Preserve sourced facts/open questions using its knowledge procedure. Checkpoint persistence failure ends local-only with retained in-memory evidence and the recovery path. Done when the complete checkpoint is available before any payload-tool call.
