...
Launch `pr-context-gatherer` and `researcher` together in one message. Skip researcher only if all five hold: no linked issues, CI is not failing, no dependency manifests/lockfiles changed, no SECURITY.md/security-related files changed, and persisted open_questions is empty. A pending upstream-behavior question always gets researcher work before R2. Done when both required tasks are launched, or the research skip and empty fields are recorded.
